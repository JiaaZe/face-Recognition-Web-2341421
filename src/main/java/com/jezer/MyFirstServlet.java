...
public class MyFirstServlet extends HttpServlet {
    ...
    float[][] featuresStored = new float[3][512];
        ...
        if ("step1".equals(step)) {
            switch (btnType) {
                case "login":
                    ...
                    /*返回登录状态*/
                    response.getWriter().println(String.valueOf(loginStatus));
                    break;
                case "register":
                    /*检测csv文件*/
                    checkCSV(basePath + "/test.csv", name, id);
                    /*返回注册状态*/
                    response.getWriter().println(String.valueOf(regStatus));
                    break;
                default:
...
//
    ...
    private void updateCSV(String path, String name, String id, float[][] feature1, float[][] feature2) {
        ...
        try {
            ...
            csvWriter.close();
            reader.close();
            new File(oldCSV).delete();
            new File(newCSV).renameTo(new File(oldCSV));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
